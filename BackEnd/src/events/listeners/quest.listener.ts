...
export class QuestListener {
  ...
  async handleQuestCreatedEvent(event: QuestCreatedEvent) {
    this.logger.log(
      `Handling quest.created for quest: ${event.title} (${event.questId})`,
    );
    try {
      await new Promise((resolve) => setTimeout(resolve, 1500));
      this.logger.log(
        `Successfully processed quest.created notification for ${event.questId}`,
      );
    } catch (error) {
      this.eventEmitter.emit('event.failed', {
        eventName: 'quest.created',
        payload: event,
        error: error.message,
      });
      throw error;
